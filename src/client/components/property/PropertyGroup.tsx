...
const PropertyGroup: React.FC<HomeGroupProps> = ({ properties, propertyTypes,agents }) => {
        ...
        <ul className="text-black flex items-center justify-center gap-5 mb-5">
          {propertyTypes.map((type, index) => (
            <li
              ...
              className={`px-6 py-2 text-[16px] cursor-pointer rounded-md ${activeType === type ? 'bg-primary text-white' : 'border border-primary text-primary'}`}
            >
              {type}
            </li>
          ))}
